...
Define the structural `ProviderAdapter` Protocol and its normalized dataclasses (per ADR-0002) that future concrete provider adapters MUST satisfy. This slice ships the contracts only; **no concrete provider adapter is implemented** (OpenAI, Anthropic, local, etc. are deferred to later slices).

> All requirements in this spec are **(planned)** per `openspec/config.yaml` `rules.specs`: the surface exists, but no concrete adapter is implemented in this slice. ADR-0002 is the source of truth for the surface; this spec fixes the contracts the provider slice must satisfy later. The Protocol and all four dataclasses ARE implemented and have passing tests — `(planned)` refers to the absence of any concrete adapter, not to the contract surface.

Source: `openspec/changes/archive/2026-07-23-core-gateway-mvp/specs/provider-abstraction/spec.md` (delta promoted to source of truth on 2026-07-23).
