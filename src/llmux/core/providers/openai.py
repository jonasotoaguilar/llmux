...
API. ``complete_stream`` is out of scope and raises ``NotImplementedError``.
The HTTP client is injected so tests can supply ``httpx.MockTransport``;
...
class OpenAIAdapter:
    ...
    async def complete(
        ...
        """POST to ``/chat/completions`` and return a ``CompletionResult``."""
    ...
    def complete_stream(
        ...
        raise NotImplementedError(
            "OpenAI streaming is out of scope; use complete() for non-streaming."
        )
    ...
    async def health(self) -> HealthStatus:
        """Probe ``GET /models``; return a ``HealthStatus`` snapshot."""
        try:
...
def _parse_completion(data: object, *, default_model: str) -> CompletionResult:
    """Translate an OpenAI chat-completion payload into a ``CompletionResult``."""
